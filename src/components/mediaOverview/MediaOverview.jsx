...
function MediaOverview() {
  const { id } = useParams();
  const { media, cast, videos, loading, error } = useMedia(id);
        ...
        {media && (
          <>
            ...
            <div className="mt-10 md:mt-0 md:ml-10 w-full md:w-1/2 lg:w-2/3 text-center">
              ...
              <hr />
              <p className="lg:text-xl mb-4 lg:p-10 pt-5 text-justify">{media.overview}</p>
              <div className="flex flex-col lg:p-10 md:flex-row justify-between">
                <p className="text-md font-semibold mb-2 md:mb-0 text-start">
                  Release Date: <span className="text-gray-400 ml-2">{media.media_type === 'movie' ? media.release_date : media.first_air_date}</span>
                </p>
                <p className="flex items-center text-md font-semibold">
                  Rating: <span className="flex text-gray-400 ml-2">{renderStars(media.vote_average)}</span>
                </p>
              </div>
              {trailerUrl? 
                
                <div className="mt-5">
                  <Button onClick={handleWatchNowClick} variant="outline" className="w-full">
                    Watch Now
                  </Button>
                </div>
        ...
        {cast && cast.length > 0 && (
          <Carousel className="w-full">
            <CarouselContent className="-ml-1">
              {cast.map((actor) => (
                <CarouselItem key={actor.cast_id} className="pl-1 md:basis-1/3 lg:basis-1/4 basis-1/2">
                  <div className="p-1">
                    <Card className="rounded-xl shadow-yellow-200 shadow-md border-none">
                      <CardContent className="flex items-center justify-center p-2">
                        <div>
                          ...
                          />
                          <p className="text-md font-semibold text-center">{actor.name}</p>
